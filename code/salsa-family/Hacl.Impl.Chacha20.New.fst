...
let u8_k : typ = P.(TBase TUInt8)
let u32_k :typ = P.(TBase TUInt32)

let uint8_p = buffer u8_k
let uint32_p = buffer u32_k

type state = b:uint32_p{length b = 16}
...
#reset-options "--z3rlimit 25" // 100

[@ "substitute"]
private
val constant_setup_:
  c:uint32_p{length c = 4} ->
  Stack unit
    (requires (fun h -> live h c))
    (ensures  (fun h0 _ h1 -> live h1 c /\ P.modifies (P.loc_buffer c) h0 h1
...
//type lbytes (n:nat) = b:uint8_p{length b == n}
//assume val uint32s_from_le: len:nat -> b:lbytes (4 * len) -> Tot (s:Seq.seq UInt32.t{length s = len}) (decreases len)

assume val uint32s_from_le_bytes:
  output:uint32_p ->
...
private
val keysetup:
  st:uint32_p{length st = 8} ->
  k:uint8_p{length k = 32 /\ disjoint st k} ->
  Stack unit
    (requires (fun h -> live h st /\ live h k))
    (ensures  (fun h0 _ h1 -> live h0 st /\ live h0 k /\ live h1 st /\ P.modifies (P.loc_buffer st) h0 h1
...
private
val ivsetup:
  st:uint32_p{length st = 3} ->
  iv:uint8_p{length iv = 12 /\ disjoint st iv} ->
  Stack unit
    (requires (fun h -> live h st /\ live h iv))
    (ensures  (fun h0 _ h1 -> live h1 st /\ P.modifies (P.loc_buffer st) h0 h1 /\ live h0 iv
      /\ (let s = reveal_h32s (as_seq h1 st) in
...
private
val ctrsetup:
  st:uint32_p{length st = 1} ->
  ctr:U32.t ->
  Stack unit
    (requires (fun h -> live h st))
    (ensures  (fun h0 _ h1 -> live h1 st /\ P.modifies (P.loc_buffer st) h0 h1
...
private val lemma_setup: h:mem -> st:state -> Lemma
  (as_seq h st == FStar.Seq.(as_seq h (BufferNG.gsub st 0ul 4ul) @| as_seq h (BufferNG.gsub st 4ul 8ul)
                           @| as_seq h (BufferNG.gsub st 12ul 1ul) @| as_seq h (BufferNG.gsub st 13ul 3ul)))
private let lemma_setup h st =
  let s = as_seq h st in
  Seq.lemma_eq_intro (Seq.slice s 0 4) (as_seq h (BufferNG.gsub st 0ul 4ul));
  Seq.lemma_eq_intro (Seq.slice s 4 12) (as_seq h (BufferNG.gsub st 4ul 8ul));
  Seq.lemma_eq_intro (Seq.slice s 12 13) (as_seq h (BufferNG.gsub st 12ul 1ul));
  Seq.lemma_eq_intro (Seq.slice s 13 16) (as_seq h (BufferNG.gsub st 13ul 3ul));
...
//   : Lemma
//     (requires (
//       UInt32.v len <= length b /\
//       live h (gsub b 0ul len) /\ (
//       live h (gsub b len (UInt32.sub (P.buffer_length b) len))
//     )))
//     (ensures (live h b))
...
val setup:
  ...
  Stack unit
    ...
    (ensures (fun h0 _ h1 -> live h0 k /\ live h0 n /\ live h1 st /\ P.modifies (P.loc_buffer st) h0 h1
      /\ (let s = reveal_h32s (as_seq h1 st) in
         let k = reveal_sbytes (as_seq h0 k) in
...
let setup st k n c =
  (**) let h0 = ST.get() in
  let stcst = BufferNG.sub st 0ul 4ul in
  let stk   = BufferNG.sub st 4ul 8ul in
  let stc   = BufferNG.sub st 12ul 1ul in
  let stn   = BufferNG.sub st 13ul 3ul in
  ...
  (**) let h4 = ST.get() in
  (**) lemma_setup h4 st;
  (**) Seq.lemma_eq_intro (reveal_h32s (as_seq h4 st))
  (**)                    (Spec.setup (reveal_sbytes (as_seq h0 k)) (reveal_sbytes (as_seq h0 n)) (U32.v c))
...
val line:
  ...
  Stack unit
    (requires (fun h -> live h st))
    (ensures (fun h0 _ h1 -> live h0 st /\ live h1 st /\ P.modifies (P.loc_buffer st) h0 h1 // BB. Could be a more precise version of modifies (st.a et st.d) using P.loc_union
...
val quarter_round:
  ...
  Stack unit
    (requires (fun h -> live h st))
    (ensures (fun h0 _ h1 -> live h0 st /\ live h1 st /\ P.modifies (P.loc_buffer st) h0 h1
...
val chacha20_core:
  ...
  Stack log_t
    ...
    (ensures  (fun h0 updated_log h1 -> live h0 st /\ live h0 k /\ live h1 k /\ invariant log h0 st
     /\ P.modifies (P.loc_union (P.loc_buffer k) (P.loc_buffer st)) h0 h1 /\ invariant updated_log h1 st
...
val alloc:
  ...
  StackInline state
    (requires (fun h -> True))
    (ensures (fun h0 st h1 -> (st `unused_in` h0) /\ live h1 st /\ frameOf st == h1.tip // /\ modifies_0 h0 h1
...
val init:
  ...
  Stack log_t
    ...
    (ensures  (fun h0 log h1 -> live h1 st /\ live h0 k /\ live h0 n /\ P.modifies (P.loc_buffer st) h0 h1
      /\ invariant log h1 st
...
assume val lemma_chacha20_counter_mode_1:
...
assume val lemma_chacha20_counter_mode_2:
  ho:mem -> output:uint8_p{live ho output} ->
  hi:mem -> input:uint8_p{live hi input} ->
...
assume val lemma_chacha20_counter_mode_0:
  ho:mem -> output:uint8_p{live ho output} ->
  hi:mem -> input:uint8_p{live hi input} ->
...
assume val lemma_xor_uint32s_to_bytes:
  in1:Seq.seq FStar.UInt8.t ->
  in2:Seq.seq u32{Seq.length in1 = 4 * Seq.length in2} ->
  Lemma (requires (True))
        (ensures (
         let out2 = Spec.Lib.uint32s_to_le (Seq.length in2) in2 in
         Spec.Lib.map2 (fun x y -> FStar.UInt8.logxor x y) in1 out2 ==
         Spec.Lib.uint32s_to_le (Seq.length in2) (Spec.Lib.map2 (fun x y -> FStar.UInt32.logxor x y) (Spec.Lib.uint32s_from_le (Seq.length in2) in1) in2)))
         (decreases (Seq.length in2))
...
val update:
  ...
  Stack log_t
    ...
    (ensures (fun h0 updated_log h1 -> live h1 output /\ live h0 plain /\ invariant updated_log h1 st
      /\ P.modifies (P.loc_union (P.loc_buffer output) (P.loc_buffer st)) h0 h1
      /\ updated_log == log
      /\ (let o = reveal_sbytes (as_seq h1 output) in
         let plain = reveal_sbytes (as_seq h0 plain) in
         match Ghost.reveal log with | MkLog k n ->
         o == L.seq_map2 (fun x y -> FStar.UInt8.(x ^^ y)) plain (chacha20_cipher k n (U32.v ctr)))))
...
//  (**)let h2 = ST.get() in
  let k  = BufferNG.sub b 0ul  16ul in
  let ib = BufferNG.sub b 16ul 16ul in
  let ob = BufferNG.sub b 32ul 16ul in
  let l  = chacha20_core log k st ctr in
//  (**)let h3 = ST.get() in
//  (**)modifies_subbuffer_2 h2 h3 k st b;
  uint32s_from_le_bytes ib plain 16ul; // slow
  (**) let h  = ST.get() in
//  (**)modifies_subbuffer_1 h3 h ib b;
  L.map2 ob ib k 16ul (fun x y -> H32.(x ^^ y));
//  (**) let h4  = ST.get() in
...
//  (**)lemma_modifies_2_1'' st output h1 h4 h5;
  lemma_xor_uint32s_to_bytes (reveal_sbytes (as_seq h0 plain)) (reveal_h32s (as_seq h k));
  pop_frame();
//  (**)let hfin = ST.get() in
...
//  ) lemma_modifies_0_2' st block h h' h'';
  let mask = BufferNG.sub block 0ul len in
  L.map2 output plain mask len (fun x y -> H8.(x ^^ y));
  (**) let h1 = ST.get() in
...
private
assume val lemma_chacha20_counter_mode_def_0:
...
val chacha20_counter_mode_blocks:
  ...
  Stack unit
    ...
    (ensures (fun h0 _ h1 -> live h1 output /\ live h0 plain /\ live h1 st
      /\ P.modifies (P.loc_union (P.loc_buffer output) (P.loc_buffer st)) h0 h1 ))// /\ invariant log h1 st))
      // /\ (let o = reveal_sbytes (as_seq h1 output) in
      //    let plain = reveal_sbytes (as_seq h0 plain) in
      //    match Ghost.reveal log with | MkLog k n ->
...
let chacha20_counter_mode_blocks output plain len log st ctr =
  let h0 = ST.get() in
  let inv (h1: mem) (i: nat): Type0 =
    live h1 output /\ live h1 st /\ P.modifies (P.loc_union (P.loc_buffer output) (P.loc_buffer st)) h0 h1 /\ 0 <= i /\ i <= UInt32.v len  ///\ invariant log h1 st
(*    /\ (match Ghost.reveal log with | MkLog k n ->
  ...
  = let h = ST.get() in
    ...
    let o'' = BufferNG.sub output 0ul (64ul *^ i +^ 64ul) in
    let b'' = BufferNG.sub plain  0ul (64ul *^ i +^ 64ul) in
    let b  = BufferNG.sub plain (64ul *^ i) 64ul in
    let b' = BufferNG.sub plain 0ul (64ul *^ i)  in
    let o  = BufferNG.sub output (64ul *^ i) 64ul in
    let o' = BufferNG.sub output 0ul (64ul *^ i)  in
...
//    no_upd_lemma_2 h h' o st o';
    ...
    //                    (as_seq h' (BufferNG.sub output 0ul (64ul *^ i +^ 64ul)));
    // Seq.lemma_eq_intro (Seq.slice (as_seq h0 plain) 0 (64 * v i + 64))
    //                    (as_seq h0 (BufferNG.sub plain 0ul (64ul *^ i +^ 64ul)));
  ...
  in
  // let o0 = BufferNG.sub output 0ul 0ul in
  // let i0 = BufferNG.sub plain  0ul 0ul in
...
//   Math.Lemmas.lemma_div_mod (UInt32.v len) 64;
//   ) let h0 = ST.get() in
//   let output' = BufferNG.sub output 0ul (64ul *^ blocks_len) in
//   let plain'  = BufferNG.sub plain  0ul (64ul *^ blocks_len) in
//   let output'' = BufferNG.sub output (64ul *^ blocks_len) part_len in
//   let plain''  = BufferNG.sub plain  (64ul *^ blocks_len) part_len in
